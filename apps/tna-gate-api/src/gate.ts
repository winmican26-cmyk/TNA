...
export class Gate {
  constructor(public readonly store: Store, private readonly now: () => number = Date.now) {}
  ...
  approve(principal: Principal, input: unknown): unknown {
    ...
    return this.store.transaction(() => {
      ...
      const approval: Approval = { id: newId('apr'), agentId: agent.id, policyHash: policy.hash, policyRevision: policy.revision, requestHash: requestHash(data.request), role: principal.role, expiresAt: data.expiresAt, used: false };
  ...
  authorize(principal: Principal, input: unknown): Decision {
    ...
    return this.store.transaction(() => {
      ...
      const decision: Decision = { ...verdict, decisionId: newId('dec'), agentId: agent.id,
