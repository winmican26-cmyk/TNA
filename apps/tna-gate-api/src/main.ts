...
const capabilityKey = process.env.TNA_CAPABILITY_KEY ?? '';
const port = Number(process.env.PORT ?? 4317);
if (!Number.isInteger(port) || port < 1 || port > 65535) throw new Error('Invalid PORT');
mkdirSync(data, { recursive: true });
const store = new Store(resolve(data, 'tna.sqlite'));
const gate = new Gate(store);
const broker = capabilityKey.length >= 32 ? new ExecutionBroker(store, new CapabilityCodec(Buffer.from(capabilityKey, 'utf8')), createDemoRegistry(resolve(data, 'demo-artifacts')), {
